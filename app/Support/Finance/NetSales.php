...
class NetSales
{
    ...
    public static function netSalesBetween(CarbonInterface $from, CarbonInterface $to): float
    {
        $sub = DB::table('transactions as t')
            ...
            ->when(auth()->check(), fn($q) => $q->where('t.cabang_id', auth()->user()->cabang_id))
    ...
    public static function grossSalesBetween(CarbonInterface $from, CarbonInterface $to): float
    {
        $sub = DB::table('transactions as t')
            ->join('transaction_items as ti', 't.id', '=', 'ti.transaction_id')
            ->whereIn('t.payment_status', self::postedPaymentStatuses())
            ...
            ->selectRaw('t.id as tx_id')
            ->selectRaw('COALESCE(t.refunded_amount, 0) as refunded_amount')
            ->selectRaw('COALESCE(t.tax_amount, 0) as tax_amount') // Ambil pajak
            ->selectRaw('COALESCE(SUM(' . self::itemNetExpr('ti') . '), 0) as item_net')
            ->groupBy('tx_id', 'refunded_amount', 'tax_amount', 'service_amount');

        // Rumus Gross: (Net Items + Pajak) - Refund
        $revenue = DB::query()
            ->fromSub($sub, 'x')
            ->selectRaw('COALESCE(SUM((x.item_net + x.tax_amount + x.service_amount) - x.refunded_amount), 0) as gross_revenue')
            ->value('gross_revenue');
