...
class PosPage extends Component
{
    ...
    //     $isDineIn = $this->orderType === 'dine_in';
    //     $isFinalPayment = ($this->orderType === 'quick_service' || ($isDineIn && $this->editingTransactionId !== null));
    ...
    //         $trx->forceFill([
    //             'payment_method' => $validated['paymentMethod'],
    //             'payment_status' => $isDineIn && $previousPaymentStatus === 'pending' ? 'paid' : ($isDineIn ? 'pending' : 'paid'),
    //             'cash_received' => $cashReceived,
    //             'cash_change' => $isCash ? max(0, $cashReceived - $this->total) : null,
    //         ])->save();

    //         // JIKA DINE IN BARU (Belum Bayar)
    //         if ($isDineIn && $trx->payment_status === 'pending') {
    //             DiningTable::where('id', $this->selectedTableId)->update(['status' => 'occupied']);
    //         }

    //         // JIKA PELUNASAN (Dine In yang tadinya occupied sekarang dibayar)
    //         if ($isDineIn && $trx->payment_status === 'paid') {
    //             DiningTable::where('id', $trx->dining_table_id)->update(['status' => 'available']);
    //         }
    ...
    public function checkout(): void
    {
        ...
        DB::transaction(function () use ($isDineIn, $isFinalPayment, $validated, &$trxId, &$previousPaymentStatus) {
            ...
            if ($isDineIn) {
                ...
                if ($isFinalPayment) {
                    \App\Models\DiningTable::where('id', $targetTableId)->update(['status' => 'available', 'occupied_at' => null]);
                } else {
                    \App\Models\DiningTable::where('id', $targetTableId)->update(['status' => 'occupied', 'occupied_at' => now()]);
                }
            }
