                    ...
                    <div
                            ...
                            <div class="space-y-2">
                                @forelse ($cartItems as $idx => $item)
                                    <div class="flex items-start gap-3"
                                        wire:key="cart-item-{{ $item['variant_id'] ?? ($item['product_id'] ?? $idx) }}">
                        ...
                        <div class="space-y-1 ">
                            ...
                            <div class="grid grid-cols-3 w-full divide-x divide-black/20 ">
                                ...
                                {{-- 2. KONDISI TAKE AWAY YANG SUDAH DI-SAVE ORDER (EDITING TRANSACTION) --}}
                            @elseif ($orderType === 'take_away' && $editingTransactionId !== null)
                                {{-- Mengisi sisa 2 kolom di baris atas --}}
                                <div class="col-span-2">
                                    <button type="button" wire:click="saveAsPending" @disabled(count($cartItems) === 0)
                                ...
                                {{-- Baris Bawah: Menampilkan Print Bill, Split Bill, dan Payment --}}
                                <div
...
<div class="grid grid-cols-1 gap-2 p-2 md:grid-cols-12 animate-in fade-in duration-150" wire:key="view-mode-payment">
    ...
    <div class="md:col-span-9 space-y-1">
        ...
        <div class=" border bg-gray-100 rounded-sm">
            <div class="flex items-center justify-between p-2 ">
                ...
                <div class="flex items-center ">
                    <div class="flex items-center justify-between h-9  gap-1">
                        ...
                        <span class="text-xs font-bold text-gray-800 whitespace-nowrap">{{ $paymentPage }}
                            of 1</span>
                        <div class="flex items-center justify-between h-11 px-2 gap-3">
                            <button type="button" wire:click="previousPage"
                                class="p-3 rounded-sm text-white bg-brand-500 border border-gray-300 rounded-lg transition">
            ...
            <div class="bg-white shadow-sm rounded-b-sm p-2 pb-40">
                @if (!$cardSubMenuOpen)
                    {{-- TAMPILAN NORMAL: Grid Payment Method Utama --}}
                    @php
                        $methods = [
                ...
                @else
                    {{-- TAMPILAN SUB-MENU: Pilihan Metode Card --}}
                    @php
                        $cardMethods = [
                            [
                                'id' => 'card',
                    ...
                    <div class="grid grid-cols-6 gap-1 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4">
                        @foreach ($cardMethods as $method)
                            <button type="button"
                                @if ($method['action'] === 'card_detail') wire:click="toggleCardSubMenu"
                    @else
                        wire:click="$set('paymentMethod', '{{ $method['id'] }}'); $set('selectedPaymentLabel', '{{ $method['name'] }} Payment'); $set('paymentModalOpen', true);" @endif
                                class="group flex min-h-[150px] w-full flex-col items-center justify-center overflow-hidden rounded-xs border border-transparent shadow-sm hover:shadow-md hover:brightness-105 transition active:scale-95"
                                style="background-color: {{ $method['color'] }};">
            ...
            <div
                ...
                <div class=" text-gray-700">
                    ...
                    <div class="flex justify-end gap-2">
                        <span>Subtotal</span>
                        <span class="tabular-nums  text-black">{{ number_format($subtotal, 0, ',', '.') }}</span>
                    </div>
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        <span class="tabular-nums text-black">{{ number_format($serviceAmount, 0, ',', '.') }}</span>
                    </div>
                    ...
                    @if ($taxAmount > 0)
                        <div class="flex justify-end gap-2">
                            <span>PB1 Total</span>
                            <span class="tabular-nums text-black">{{ number_format($taxAmount, 0, ',', '.') }}</span>
                        </div>
                    @endif
                    {{-- Compliment --}}
                ...
                <div class="text-gray-700">
                    <div class="flex justify-end gap-2">
                        <span>Billing Total</span>
                        <span class="tabular-nums text-black">{{ number_format($total, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-end gap-2">
                        <span>Voucher Purchase</span>
                        <span class="tabular-nums text-black">0</span>
                            ...
                            @if (isset($splitBills[$activeSplitTab]) && count($splitBills[$activeSplitTab]['items']) > 0)
                                @foreach ($splitBills[$activeSplitTab]['items'] as $sIdx => $sItem)
                                    <div class="flex items-center justify-between p-1  rounded-lg"
                                        wire:key="split-bill-{{ $activeSplitTab }}-{{ $sItem['variant_id'] ?? ($sItem['product_id'] ?? $sIdx) }}">
                                        <div class="min-w-0 flex-1">
            ...
            <div class="flex items-center justify-end px-5 py-2 border-t border-gray-100 text-xs text-gray-500 gap-2">
                @if ($this->promotions->hasPages())
                    ...
                    <button wire:click="previousPromotionPage"
                        {{ $this->promotions->onFirstPage() ? 'disabled' : '' }}
                        class="px-1 disabled:opacity-40">&lt;</button>
                    <button wire:click="nextPromotionPage" {{ $this->promotions->hasMorePages() ? '' : 'disabled' }}
                        class="px-1 disabled:opacity-40">&gt;</button>
            ...
            <div
                ...
                <div
                    ...
                    <button wire:click="$set('voucherListPage', {{ max(1, $voucherListPage - 1) }})"
                        ...
                        class="w-7 h-7 flex items-center justify-center border rounded disabled:opacity-30 hover:bg-gray-50">‹</button>
                    <button wire:click="$set('voucherListPage', {{ min($vouchers->lastPage(), $voucherListPage + 1) }})"
