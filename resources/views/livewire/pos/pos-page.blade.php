<div class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-12 lg:items-center">
                ...
                {{-- <div class="col-span-2 sm:col-span-1 lg:col-span-3">
                    <div class="flex gap-2">
                        <input wire:model.live="scanCode" type="text" placeholder="Kode transaksi"
                            class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm dark:bg-gray-900 dark:text-white" />
                        <button type="button" wire:click="importTransactionCode"
                            class="bg-brand-500 shrink-0 rounded-lg px-4 text-sm font-semibold text-white">Proses</button>
                    </div>
                </div> --}}
            </div>
        </div>
    ...
    {{-- 2. KONTEN UTAMA & KETIKA PERTAMA KALI DINE IN DAN BELUM MILIH MEJA --}}
    @if ($orderType === 'dine_in' && !$selectedTableId)
        {{-- VIEW UTAMA (Layout 2 Kolom) --}}
        <div class="grid grid-cols-1 gap-6 p-2 md:grid-cols-12">

            {{-- BAGIAN KIRI: Berubah antara List Meja atau List Produk --}}
            <div class="md:col-span-8">

                {{-- KONDISI 1: Mode Dine In dan Belum Pilih Meja --}}
                @if ($orderType === 'dine_in' && !$selectedTableId)
                    <div class="mb-4">
                    ...
                    {{-- @if ($totalPages > 1)
                        <div class="flex flex-wrap gap-1 mb-4">
                            @for ($i = 1; $i <= $totalPages; $i++)
                                <button type="button" wire:click="$set('productPage', {{ $i }})"
                                    @class([
                                        'px-4 py-2 text-xs font-bold rounded-md border transition-all',
                                        'bg-brand-500 text-white border-brand-600' => $productPage == $i,
                                        'bg-white text-gray-600 border-gray-200 hover:border-brand-500 dark:bg-gray-900 dark:border-gray-700 dark:text-gray-400' =>
                                            $productPage != $i,
                                    ])>{{ $i }}</button>
                            @endfor
