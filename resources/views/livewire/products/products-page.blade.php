<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div
            ...
            <button type="button" wire:click="setTab('products')" @class([
            ...
            @can('categories.view')
                <button type="button" wire:click="setTab('categories')" @class([
                    'rounded-md px-4 py-2 text-sm font-medium transition',
        ...
        <div class="fixed inset-0 z-[100000] flex items-center justify-center p-4" aria-modal="true" role="dialog">
            ...
            <div
                                ...
                                <table class="w-full text-sm text-left">
                                    <thead class="bg-gray-50 dark:bg-white/[0.03] sticky top-0">
                                        <tr>
                                            <th class="p-3 w-10">
                                                <input type="checkbox"
                                                    x-on:click="$el.checked ? $wire.set('selectedSourceProductIds', @js($this->sourceProducts->pluck('id'))) : $wire.set('selectedSourceProductIds', [])"
                                    ...
                                    <tbody class="divide-y dark:divide-gray-800">
                                        @forelse($this->sourceProducts as $sourceProd)
                                            <tr class="dark:text-gray-300">
                                                ...
                                                <td class="p-3 font-medium">{{ $sourceProd->name }}</td>
                                                <td class="p-3">{{ $sourceProd->category->name ?? '-' }}</td>
                                                <td class="p-3 text-right">
                                                    Rp{{ number_format($sourceProd->variants->min('price'), 0, ',', '.') }}
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                ...
                <div
                    ...
                    <button wire:click="copyMenu" @disabled(empty($selectedSourceProductIds))
                        class="px-4 py-2 text-sm font-semibold text-white bg-brand-500 rounded-lg hover:bg-brand-600 disabled:opacity-50">
                        Konfirmasi Copy ({{ count($selectedSourceProductIds) }} Menu)
                    </button>
                </div>
            </div>
        </div>
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="inline-flex items-center gap-2">
                                        ...
                                        @else
                                            @can('categories.edit')
                                                <button type="button"
                                                    wire:click="startEditCategory({{ (int) $category->id }})"
                                                    class="shadow-theme-xs inline-flex items-center justify-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03]">
                                                    Edit
                                                </button>
                                            @endcan
                                            @can('categories.delete')
                                                <button type="button"
                                                    x-on:click.prevent="$dispatch('confirm', { message: 'Hapus kategori ini?', method: 'deleteCategory', args: [{{ (int) $category->id }}] })"
                                                    class="shadow-theme-xs inline-flex items-center justify-center rounded-lg border border-gray-300 bg-white px-3 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03]">
                                                    Hapus
                                                </button>
                                            @endcan
                    ...
                    <form wire:submit="createCategory" class="space-y-4 p-5">
                        <div>
                            ...
                            <input wire:model.live="categoryName" type="text"
                                aria-invalid="{{ $errors->has('categoryName') ? 'true' : 'false' }}"
                                aria-describedby="{{ $errors->has('categoryName') ? 'error-categoryName' : '' }}"
