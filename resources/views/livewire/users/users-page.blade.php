        ...
        <div class="custom-scrollbar overflow-x-auto">
            <table class="w-full table-auto">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    @empty
                        <x-common.empty-table-row colspan="6" message="Belum ada pengguna." />
                    @endforelse
                </tbody>
            </table>
        </div>
                ...
                <form wire:submit="createUser" autocomplete="off" class="space-y-4 p-5">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <select wire:model.live="cabangId" aria-invalid="{{ $errors->has('cabangId') ? 'true' : 'false' }}" aria-describedby="{{ $errors->has('cabangId') ? 'error-cabangId' : '' }}" class="shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-white px-4 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">
                                <option value="">Pilih cabang</option>
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch->id }}">{{ $branch->name }}</option>
                                @endforeach
                            </select>
                            <x-common.input-error for="cabangId" />
                        </div>
                        <div>
            ...
            <div class="relative w-full max-w-2xl overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl dark:border-gray-800 dark:bg-gray-900">
                ...
                <form wire:submit="updateUser" autocomplete="off" class="space-y-4 p-5">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">Cabang</label>
                            <select wire:model.live="editingCabangId" aria-invalid="{{ $errors->has('editingCabangId') ? 'true' : 'false' }}" aria-describedby="{{ $errors->has('editingCabangId') ? 'error-editingCabangId' : '' }}" class="shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-white px-4 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">
                                <option value="">Pilih cabang</option>
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch->id }}">{{ $branch->name }}</option>
                                @endforeach
                            </select>
                            <x-common.input-error for="editingCabangId" />
                        </div>
                        <div>
                            <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">Peran</label>
