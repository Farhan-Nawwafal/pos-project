    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                {{ optional($transaction->created_at)->format('d M Y, H:i') }} · {{ $paymentMethodLabel }} ·
                {{ $paymentStatusLabel }}
            </p>
        </div>
            ...
            <dl class="mt-4 space-y-3">
                ...
                <div class="flex items-start justify-between gap-4">
                    ...
                    <dd class="text-sm font-medium text-gray-800 dark:text-white/90 text-right">
                        {{ $orderType === 'dine_in' ? 'Dine in' : 'Take away' }}</dd>
                </div>
                <div class="flex items-start justify-between gap-4">
                    <dt class="text-sm text-gray-500 dark:text-gray-400">Meja</dt>
                    <dd class="text-sm font-medium text-gray-800 dark:text-white/90 text-right">
                        {{ $transaction->diningTable?->name ?? '-' }}</dd>
                </div>
                <div class="flex items-start justify-between gap-4">
                    <dt class="text-sm text-gray-500 dark:text-gray-400">Metode</dt>
                    <dd class="text-sm font-medium text-gray-800 dark:text-white/90 text-right">
                        {{ $paymentMethodLabel }}</dd>
                </div>
                <div class="flex items-start justify-between gap-4">
                    <dt class="text-sm text-gray-500 dark:text-gray-400">Status</dt>
                    <dd class="text-sm font-medium text-gray-800 dark:text-white/90 text-right">
                        {{ $paymentStatusLabel }}</dd>
                </div>
                @if ($discountTotal > 0)
                    <div class="flex items-start justify-between gap-4">
                        <dt class="text-sm text-gray-500 dark:text-gray-400">Subtotal Bersih</dt>
                        <dd class="text-sm font-medium text-gray-800 dark:text-white/90 text-right">
                            {{ $fmtCurrency($netSubtotal) }}</dd>
                    </div>
                @endif
                @if ((int) ($transaction->service_amount ?? 0) > 0)
                    <div class="flex items-start justify-between gap-4">
                        <dt class="text-sm text-gray-500 dark:text-gray-400">Service Charge</dt>
                        <dd class="text-sm font-medium text-gray-800 dark:text-white/90 text-right">
                            {{ $fmtCurrency((int) $transaction->service_amount) }}
                            <span
                                class="text-[10px] text-gray-400">({{ number_format((float) $transaction->service_percentage, 0) }}%)</span>
                        </dd>
                    </div>
                @endif
                <div class="flex items-start justify-between gap-4">
                    ...
                    <dd class="text-sm font-medium text-gray-800 dark:text-white/90 text-right">
                        {{ $fmtCurrency((int) ($transaction->refunded_amount ?? 0)) }}</dd>
                </div>
                    ...
                    <div class="flex items-start justify-between gap-4">
                        ...
                        <dd class="text-sm font-medium text-gray-800 dark:text-white/90 text-right">
                            ...
                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                {{ $transaction->voucherCampaign?->name ?? '-' }} ·
                                -{{ $fmtCurrency((int) ($transaction->voucher_discount_amount ?? 0)) }}
                            </div>
                        </dd>
                    </div>
                @endif
                        ...
                        <dd class="text-sm font-medium text-gray-800 dark:text-white/90 text-right">
                            ...
                            @if ($transaction->manualDiscountByUser)
                                <div class="text-xs text-gray-500 dark:text-gray-400">Oleh:
                                    {{ $transaction->manualDiscountByUser->name }}</div>
                            @endif
                        </dd>
                    </div>
                @endif
                        ...
                        <dd class="text-sm font-medium text-gray-800 dark:text-white/90 text-right">
                            ...
                            @if ((int) ($transaction->points_redeemed ?? 0) > 0)
                                <div class="text-xs text-gray-500 dark:text-gray-400">Dipakai:
                                    {{ number_format((int) ($transaction->points_redeemed ?? 0), 0, ',', '.') }}</div>
                            @endif
                            @if ((int) ($transaction->points_earned ?? 0) > 0)
                                <div class="text-xs text-gray-500 dark:text-gray-400">Didapat:
                                    {{ number_format((int) ($transaction->points_earned ?? 0), 0, ',', '.') }}</div>
                            @endif
                        </dd>
                    </div>
                @endif
                <div class="flex items-start justify-between gap-4">
                    ...
                    <dd class="text-right">
                        <span
                            class="rounded-full px-2 py-0.5 text-theme-xs font-medium {{ $inventoryApplied ? 'bg-success-50 text-success-600 dark:bg-success-500/15 dark:text-success-500' : 'bg-warning-50 text-warning-600 dark:bg-warning-500/15 dark:text-orange-400' }}">
                ...
                <div class="flex items-start justify-between gap-4">
                    ...
                    <dd class="text-sm font-medium text-gray-800 dark:text-white/90 text-right">
                        {{ $fmtCurrency((int) ($transaction->cash_received ?? 0)) }}</dd>
                </div>
                <div class="flex items-start justify-between gap-4">
                    <dt class="text-sm text-gray-500 dark:text-gray-400">Kembalian</dt>
                    <dd class="text-sm font-medium text-gray-800 dark:text-white/90 text-right">
                        {{ $fmtCurrency((int) ($transaction->cash_change ?? 0)) }}</dd>
                </div>
                <div class="flex items-start justify-between gap-4">
                            ...
                            <tr>
                                <td class="px-5 py-4">
                                    <div class="space-y-1">
                                        <p class="text-sm font-medium text-gray-800 dark:text-white/90">
                                            {{ $item->product?->name ?? '-' }}</p>
                                        @if ($item->note)
                                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $item->note }}
                                            </p>
                                        @endif
                                            ...
                                            <div class="space-y-0.5">
                                                @foreach ($children as $child)
                                                    ...
                                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                                        •
                                                        {{ (string) ($child->product?->name ?? 'Produk') }}{{ $childVariant !== '' ? ' - ' . $childVariant : '' }}
                                                        x{{ number_format((int) ($child->quantity ?? 0), 0, ',', '.') }}
                                                    </p>
                                                @endforeach
                                            </div>
                                        @endif
                                ...
                                <td class="px-5 py-4">
                                    <p class="text-sm text-gray-800 dark:text-white/90">
                                        {{ $variantDisplay !== '' ? $variantDisplay : '-' }}</p>
                                </td>
                                <td class="px-5 py-4 text-right">
                                    <p class="text-sm font-medium text-gray-800 dark:text-white/90">
                                        {{ number_format((int) $item->quantity, 0, ',', '.') }}</p>
                                </td>
                                <td class="px-5 py-4 text-right">
                                    <p class="text-sm text-gray-800 dark:text-white/90">
                                        {{ $fmtCurrency((float) $item->price) }}</p>
                                </td>
                                <td class="px-5 py-4 text-right">
                                    <p class="text-sm font-semibold text-gray-800 dark:text-white/90">
                                        {{ $fmtCurrency((float) $item->subtotal) }}</p>
                                    @if ($voucherItemDiscount > 0 || $manualItemDiscount > 0)
                                        <p class="text-xs text-gray-500 dark:text-gray-400">
                                            ...
                                            @if ($manualItemDiscount > 0)
                                                {{ $voucherItemDiscount > 0 ? '·' : '' }} Manual
                                                {{ $fmtCurrency($manualItemDiscount) }}
                                            @endif
                                        </p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400">Net:
                                            {{ $fmtCurrency($netLineSubtotal) }}</p>
                                    @endif
                                </td>
                                <td class="px-5 py-4 text-right">
                                    <p class="text-sm text-gray-800 dark:text-white/90">
                                        {{ $inventoryApplied ? $fmtCurrency($hppTotal) : '-' }}</p>
                                </td>
                                <td class="px-5 py-4 text-right">
                                    <p
                                        class="text-sm font-semibold {{ $hppTotal > 0 && $profit < 0 ? 'text-error-600 dark:text-error-500' : 'text-gray-800 dark:text-white/90' }}">
            ...
            <div class="border-t border-gray-200 px-5 py-4 dark:border-gray-800">
                <div class="flex flex-col gap-2 text-sm">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span
                            class="font-medium text-gray-800 dark:text-white/90">{{ $fmtCurrency((int) $transaction->subtotal) }}</span>
                    </div>
                    @if ($discountTotal > 0)
                        <div class="flex items-center justify-between">
                            <span class="text-gray-500 dark:text-gray-400">Total Diskon</span>
                            <span
                                class="font-medium text-gray-800 dark:text-white/90">-{{ $fmtCurrency($discountTotal) }}</span>
                        </div>
                    @endif
                    @if ((int) ($transaction->voucher_discount_amount ?? 0) > 0)
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                class="font-medium text-gray-800 dark:text-white/90">-{{ $fmtCurrency((int) $transaction->voucher_discount_amount) }}</span>
                        </div>
                    @endif
                    @if ((int) ($transaction->manual_discount_amount ?? 0) > 0)
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                class="font-medium text-gray-800 dark:text-white/90">-{{ $fmtCurrency((int) $transaction->manual_discount_amount) }}</span>
                        </div>
                    @endif
                    @if ($pointDiscountAmount > 0)
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                class="font-medium text-gray-800 dark:text-white/90">-{{ $fmtCurrency($pointDiscountAmount) }}</span>
                        </div>
                    @endif
                    @if ($discountTotal > 0)
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                class="font-medium text-gray-800 dark:text-white/90">{{ $fmtCurrency((int) $netSubtotal) }}</span>
                        </div>
                    @endif
                    @if ((int) ($transaction->points_redeemed ?? 0) > 0 || (int) ($transaction->points_earned ?? 0) > 0)
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                class="font-medium text-gray-800 dark:text-white/90">{{ number_format((int) ($transaction->points_redeemed ?? 0), 0, ',', '.') }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-gray-500 dark:text-gray-400">Poin Didapat</span>
                            <span
                                class="font-medium text-gray-800 dark:text-white/90">{{ number_format((int) ($transaction->points_earned ?? 0), 0, ',', '.') }}</span>
                        </div>
                    @endif
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Total HPP</span>
                        <span
                            class="font-medium text-gray-800 dark:text-white/90">{{ $inventoryApplied ? $fmtCurrency($totalHpp) : 'Belum diproses' }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Estimasi Laba</span>
                        <span
                            class="font-medium text-gray-800 dark:text-white/90">{{ $inventoryApplied ? $fmtCurrency($totalProfit) : '-' }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Service Charge
                            ({{ number_format((float) ($transaction->service_percentage ?? 0), 0) }}%)</span>
                        <span
                            class="font-medium text-gray-800 dark:text-white/90">{{ $fmtCurrency((int) ($transaction->service_amount ?? 0)) }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Pajak PB1</span>
                        <span
                            class="font-medium text-gray-800 dark:text-white/90">{{ $fmtCurrency((int) ($transaction->tax_amount ?? 0)) }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Biaya Admin</span>
                        <span
                            class="font-medium text-gray-800 dark:text-white/90">{{ $feeAmount > 0 ? $fmtCurrency($feeAmount) : '-' }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Pembulatan</span>
                        <span
                            class="font-medium text-gray-800 dark:text-white/90">{{ $fmtCurrency($roundingAmount) }}</span>
                    </div>
                    <div class="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-800">
                        <span class="text-gray-800 dark:text-white/90 font-semibold">Total</span>
                        <span
                            class="text-gray-800 dark:text-white/90 font-semibold">{{ $fmtCurrency((int) $transaction->total) }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="px-5 py-4">
                    <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-center gap-2">
                            <span
                                class="rounded-full px-2 py-0.5 text-theme-xs font-medium {{ $event->action === 'refund' ? 'bg-warning-50 text-warning-600 dark:bg-warning-500/15 dark:text-orange-400' : ($event->action === 'void' ? 'bg-gray-50 text-gray-700 dark:bg-gray-500/15 dark:text-gray-300' : 'bg-success-50 text-success-600 dark:bg-success-500/15 dark:text-success-500') }}">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            {{ optional($event->created_at)->format('d M Y, H:i') }}</p>
                    </div>
                    ...
                    <div class="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
                        ...
                        @if ($event->action === 'manual_discount')
                            ...
                            @if (!empty($meta['type']) && !empty($meta['value']))
                                <p class="text-sm text-gray-700 dark:text-gray-300"><span
                                    ...
                                    {{ $meta['type'] === 'percent' ? ((int) $meta['value']) . '%' : $fmtCurrency((int) $meta['value']) }}
                                </p>
                            @endif
                        @endif
                        ...
                        @if (!empty($meta['previous_payment_status']) || !empty($meta['new_payment_status']))
                            <p class="text-sm text-gray-700 dark:text-gray-300"><span
                                ...
                                {{ \App\Helpers\DataLabelHelper::enum($meta['previous_payment_status'] ?? null, 'payment_status') }}
                                →
                                {{ \App\Helpers\DataLabelHelper::enum($meta['new_payment_status'] ?? null, 'payment_status') }}
                            </p>
                        @endif
                        @if (array_key_exists('revert_inventory', $meta))
                            <p class="text-sm text-gray-700 dark:text-gray-300"><span
                                    class="text-gray-500 dark:text-gray-400">Revert stok:</span>
                                {{ (bool) $meta['revert_inventory'] ? 'Ya' : 'Tidak' }}</p>
                        @endif
                    </div>
                </div>
            @empty
                    ...
                    <form wire:submit="voidTransaction" class="space-y-4 p-5">
                        <div>
                            ...
                            <input wire:model.live="correctionReason" type="text"
                                aria-invalid="{{ $errors->has('correctionReason') ? 'true' : 'false' }}"
                                aria-describedby="{{ $errors->has('correctionReason') ? 'error-correctionReason' : '' }}"
                        ...
                        @if ($voidNeedsApproval)
                            <div
                                ...
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                    ...
                                    {{ number_format($voidQuickMaxCount, 0, ',', '.') }}/hari dan window
                                    {{ number_format($voidWindowMinutes, 0, ',', '.') }} menit.
                                </p>
                                ...
                                <div class="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                                    <div>
                                        ...
                                        <select wire:model.live="approverUserId"
                                            aria-invalid="{{ $errors->has('approverUserId') ? 'true' : 'false' }}"
                                            aria-describedby="{{ $errors->has('approverUserId') ? 'error-approverUserId' : '' }}"
                                    ...
                                    <div>
                                        ...
                                        <input wire:model.live="approverPin" type="password" inputmode="numeric"
                                            aria-invalid="{{ $errors->has('approverPin') ? 'true' : 'false' }}"
                                            aria-describedby="{{ $errors->has('approverPin') ? 'error-approverPin' : '' }}"
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Batas refund cepat:
                                Rp{{ number_format($refundQuickMaxAmount, 0, ',', '.') }} (maks
                                {{ number_format($refundQuickMaxCount, 0, ',', '.') }}/hari/kasir). Jika melebihi,
                                sistem minta PIN.</p>
                        </div>
