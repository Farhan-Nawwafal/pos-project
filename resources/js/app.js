...
const initWidgets = () => {
    if (document.querySelector("#memberMap")) {
        import("./components/member-map").then((module) =>
            module.initMemberMap(),
        );
    }
...
document.addEventListener("livewire:init", () => {
    if (window.Livewire?.on) {
        ...
        const getChartTwoModule = () =>
            (chartTwoModulePromise ??= import("./components/chart/chart-2"));
        const getChartThreeModule = () =>
            (chartThreeModulePromise ??= import("./components/chart/chart-3"));
        const getMemberMapModule = () =>
            (memberMapModulePromise ??= import("./components/member-map"));
        ...
        window.Livewire.on("printer-sources-updated", (data) => {
            const payload =
                data?.detail ?? (Array.isArray(data) ? data[0] : data) ?? {};
            ...
            window.dispatchEvent(
                new CustomEvent("printer-sources-updated", {
                    detail: { sources: window.PRINTER_SOURCES },
                }),
            );
        });
    }
...
document.addEventListener("alpine:init", () => {
    Alpine.data("currencyInput", (entangle) => ({
        ...
        formatDisplay() {
            ...
            this.displayValue = new Intl.NumberFormat("id-ID").format(
                this.value,
            );
        },
