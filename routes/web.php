...
$rootRedirect = function () {
    if (User::exists()) {
        // Kalau sudah ada user (owner), arahkan ke login
        return redirect()->route('signin');
    }
    // Kalau database masih kosong, arahkan ke setup
    return redirect()->route('setup');
};

if (is_string($landingDomain) && $landingDomain !== '') {
    Route::domain($landingDomain)->group(function () use ($rootRedirect) {
