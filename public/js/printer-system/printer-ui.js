    ...
    const registerPrinterSystem = () => {
        if (!Alpine.data("printerSystem")) {
            Alpine.data("printerSystem", () => ({
                printers: {},
                isOpen: false,
                sources: Array.isArray(window.PRINTER_SOURCES)
                    ? window.PRINTER_SOURCES
                    : [],
                ...
                init() {
                    ...
                    this.onRestored = (event) => {
                        ...
                        this.printers[role].deviceName =
                            device?.name || "Printer Tidak Ditemukan";

                        const isConnected =
                            window.PrinterManager.isPrinterReady(role);
                        this.printers[role].status = isConnected
                            ? "Terhubung"
                            : "Tidak Terhubung";
                    };
                    ...
                    this.onSourcesUpdated = (event) => {
                        ...
                        this.sources = Array.isArray(nextSources)
                            ? nextSources
                            : Array.isArray(window.PRINTER_SOURCES)
                              ? window.PRINTER_SOURCES
                              : [];

                        if (this.sources.length > 0) {
                            window.PrinterManager.configureSources(
                                this.sources,
                            );
                            this.buildPrinters();
                            this.syncFromManager();
                    ...
                    this.onDisconnect = (event) => {
                        ...
                        Object.keys(this.printers).forEach((role) => {
                            if (
                                window.PrinterManager.devices[role] === device
                            ) {
                                this.printers[role].status = "Tidak Terhubung";
                            }
                        });
                    ...
                    this.onConnect = (event) => {
                        ...
                        if (!role || !this.printers[role]) return;
                        this.printers[role].deviceName =
                            device?.name || this.printers[role].deviceName;
                ...
                syncFromManager() {
                    Object.keys(this.printers).forEach((role) => {
                        ...
                        const connected =
                            window.PrinterManager.isPrinterReady(role);
                        this.printers[role].deviceName =
                            device?.name || "Belum Dikonfigurasi";
                        this.printers[role].status = connected
                            ? "Terhubung"
                            : "Tidak Terhubung";
                    });
                },
                ...
                async setup(role) {
                    const success =
                        await window.PrinterManager.setupPrinter(role);
                    if (success) {
                        const device = window.PrinterManager.devices[role];
                ...
                testPrint(role) {
                    const dummyOrder = {
                        ...
                        order: {
                            ...
                            subtotal: 10000,
                            service_percentage: 10, // Tambahkan ini (sesuaikan default)
                            tax_percentage: 10, // Tambahkan ini (sesuaikan default)
                            service_amount: 1000,
                            tax_amount: 1000,
                        },
                        items: [
                    ...
                    if (type === "kasir") {
                        const data =
                            window.ReceiptTemplates.customerCopy(dummyOrder);
                        window.PrintQueue.add({ printerRole: role, data });
                    } else {
                        const label = config?.label || "KITCHEN";
                        const data = window.ReceiptTemplates.kitchenCopy(
                            dummyOrder,
                            label.toUpperCase(),
                        );
                        window.PrintQueue.add({ printerRole: role, data });
            ...
            if (window.Livewire?.on) {
                window.Livewire.on("pos-print-modal", (data) => {
                    ...
                    window.dispatchEvent(
                        new CustomEvent("pos-print-modal", { detail: payload }),
                    );
                });
            }
        }
        ...
        if (!Alpine.data("posPrintModal")) {
            Alpine.data("posPrintModal", () => ({
                ...
                init() {
                    this.onModal = (data) => {
                        ...
                        if (window.PRINTER_DEBUG) {
                            try {
                                console.groupCollapsed(
                                    "[POS Print] event received: pos-print-modal",
                                );
                                ...
                                console.info(
                                    ...
                                    payload?.items_by_source
                                        ? Object.keys(payload.items_by_source)
                                        : null,
                                );
                                console.groupEnd();
                            } catch (e) {}
                        ...
                        if (
                            ...
                            payload?.default_kasir_source_id
                        ) {
                            selected[
                                this.roleFromSourceId(
                                    payload.default_kasir_source_id,
                                )
                            ] = true;
                        }
                ...
                sources() {
                    const list = Array.isArray(this.payload?.printer_sources)
                        ? this.payload.printer_sources
                        : Array.isArray(window.PRINTER_SOURCES)
                          ? window.PRINTER_SOURCES
                          : [];
                    return list.map((s) => ({
                ...
                itemsForSource(source) {
                    ...
                    if (type === "kasir")
                        return Array.isArray(this.payload.items)
                            ? this.payload.items
                            : [];
                    if (type === "checker")
                        return Array.isArray(this.payload.checker_items)
                            ? this.payload.checker_items
                            : [];
                ...
                selectedSourceIds() {
                    return Object.keys(this.selected || {})
                        ...
                        .map((role) =>
                            Number(String(role).replace("source-", "")),
                        )
                ...
                kasirSource() {
                    const sources = this.sources();
                    const kasir = sources.find(
                        (s) => String(s.type || "").toLowerCase() === "kasir",
                    );
                    return kasir || null;
                },
                kasirJob() {
                ...
                hasKitchenSelected() {
                    ...
                    return selectedIds.some((id) => {
                        ...
                        return (
                            s &&
                            type === "dapur" &&
                            this.itemsForSource(s).length > 0
                        );
                    });
                },
                printerStatus(source) {
                    ...
                    const savedId = localStorage.getItem(
                        `printer_device_${role}`,
                    );
                    if (!savedId)
                        return { key: "missing_config", label: "Belum Setup" };
                    if (!window.PrinterManager?.devices?.[role])
                        return { key: "permission_lost", label: "Izin Hilang" };
                    if (window.PrinterManager?.isPrinterReady?.(role))
                        return { key: "connected", label: "Terhubung" };
                ...
                statusClass(statusKey) {
                    ...
                    if (
                        statusKey === "missing_config" ||
                        statusKey === "permission_lost"
                    )
                ...
                unassignedNames() {
                    const items = Array.isArray(this.payload?.unassigned_items)
                        ? this.payload.unassigned_items
                        : [];
                    const names = items
                        .map((i) => String(i?.product?.name || i?.name || ""))
                        .filter(Boolean);
                    const uniq = {};
                    names.forEach((n) => {
                        uniq[n] = true;
                ...
                revalidate() {
                    ...
                    if (jobs.length === 0) {
                        issues.push({
                            key: "no_selection",
                            type: "no_selection",
                            message: "Pilih minimal 1 printer untuk mencetak.",
                        });
                    }

                    jobs.forEach((job) => {
                        const source = byId[job.printer_source_id];
                        const role = this.roleFromSourceId(
                            job.printer_source_id,
                        );
                        ...
                        if (!savedId) {
                            issues.push({
                                key: `${role}_missing_config`,
                                role,
                                type: "missing_config",
                                label,
                                message: "Printer belum disetup.",
                            });
                            return;
                        }
                        ...
                        if (!window.PrinterManager?.devices?.[role]) {
                            issues.push({
                                key: `${role}_permission_lost`,
                                role,
                                type: "permission_lost",
                                label,
                                message:
                                    "Izin perangkat hilang. Setup ulang diperlukan.",
                            });
                            return;
                        }
                        ...
                        return [
                            {
                                key: "no_kasir",
                                type: "no_kasir",
                                message: "Printer kasir belum dibuat.",
                            },
                        ];
                        ...
                        return [
                            {
                                ...
                                type: "missing_config",
                                label,
                                message: "Printer kasir belum disetup.",
                            },
                        ];
                    if (!window.PrinterManager?.devices?.[role])
                        return [
                        ...
                        return [
                            {
                                ...
                                type: "offline",
                                label,
                                message: "Printer kasir belum Terhubung.",
                            },
                        ];
                    return [];
                },
                ...
                blockingIssues() {
                    return (
                        Array.isArray(this.issues) ? this.issues : []
                    ).filter((issue) => {
                        const type = String(issue?.type || "");
                ...
                async printSelected() {
                    ...
                    try {
                        ...
                        if (window.PRINTER_DEBUG) {
                            try {
                                console.groupCollapsed(
                                    "[POS Print] printSelected",
                                );
                                console.info("order", orderData?.order);
                                console.info(
                                    "print_jobs",
                                    orderData?.print_jobs,
                                );
                                console.groupEnd();
                            } catch (e) {}
                        }
                        ...
                        if (!window.PrinterManager?.printOrder) {
                            console.error(
                                "[POS Print] PrinterManager tidak tersedia",
                            );
                            return;
                        }
                ...
                async printKasirOnly() {
                    ...
                    if (this.kasirIssues().length > 0) {
                        if (window.PRINTER_DEBUG)
                            console.warn("[POS Print] kasir-only blocked", {
                                issues: this.kasirIssues(),
                            });
                        return;
                    }
                    this.busy = true;
