    ...
    class PrinterManager {
        ...
        async printOrder(orderData) {
            ...
            if (Array.isArray(orderData?.print_jobs) && orderData.print_jobs.length > 0) {
                for (const job of orderData.print_jobs) {
                    ...
                    if (sourceType === 'kasir') {
                        const customerData = await window.ReceiptTemplates.customerCopy({
                            ...orderData,
                            items: jobItems
                        });
                    ...
                    const kitchenData = await window.ReceiptTemplates.kitchenCopy({
                        ...orderData,
                        items: jobItems
                    }, label);
