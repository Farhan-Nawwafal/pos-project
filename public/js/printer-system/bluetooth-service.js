...
if (!window.BluetoothService) {
    class BluetoothService {
        constructor() {
            ...
            this.chunkSize = 100; // Safe chunk size for BLE
        ...
        // }
        /**
 * Send data in chunks to avoid buffer overflow and WiFi interference.
 */
async sendChunks(characteristic, data) {
    let offset = 0;
    
    // PERUBAHAN 1: Gunakan jeda yang konsisten.
    // 30ms memberikan waktu bagi antena untuk bergantian antara Bluetooth dan WiFi.
    const delayMs = 30; 

    // PERUBAHAN 2: Kecilkan ukuran chunk jika sering putus. 
    // Nilai 20-64 lebih ramah untuk perangkat yang antenanya berbagi (shared antenna).
    const limitedChunkSize = 64; 

    while (offset < data.length) {
        const chunk = data.slice(offset, offset + limitedChunkSize);
        
        try {
            await characteristic.writeValue(chunk);
        } catch (error) {
            console.error("Gagal mengirim chunk:", error);
            throw error;
        }

        offset += limitedChunkSize;

        // PERUBAHAN 3: Pastikan jeda ini selalu dijalankan
        await new Promise(resolve => setTimeout(resolve, delayMs));
    }
}
    }
