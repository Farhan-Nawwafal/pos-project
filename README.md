...
----------------------------
1) Kalo mau migrasi databasenya, usahakan migrate dulu database cabangs baru migrate yang lainnya. Caranya:
    php artisan migrate --path=database\\migrations\\2026_04_02_174333_create_cabangs_table.php

2) Jalankan migrasi:
   php artisan migrate

3) Pastikan tabel session tersedia (karena default env memakai SESSION_DRIVER=database).
...
4) Jalankan seeder role & permission (wajib untuk akses admin):
